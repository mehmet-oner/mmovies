...
const MOVIES: Movie[] = [
  ...
  },
  {
    ...
    poster: "/posters/interstellar.svg",
  },
  {
    id: "parasite",
    title: "Parasite",
    ...
    rating: 8.5,
    description:
      "Two families’ lives intersect in a darkly comedic social thriller.",
    whereToWatch: "Hulu",
    poster: "/posters/parasite.svg",
  },
  {
    id: "darkknight",
  ...
  },
  {
    ...
    rating: 8.6,
    description:
      "A girl enters a spirit world to save her parents and herself.",
    whereToWatch: "Max",
    poster: "/posters/spiritedaway.svg",
  },
  {
    id: "madmax",
    ...
    rating: 8.1,
    description:
      "A post-apocalyptic chase for freedom across the wasteland.",
    whereToWatch: "Hulu",
    poster: "/posters/madmax.svg",
  },
  {
    id: "whiplash",
    title: "Whiplash",
    ...
    rating: 8.5,
    description:
      "An ambitious drummer faces a ruthless instructor at a music conservatory.",
    whereToWatch: "Netflix",
    poster: "/posters/whiplash.svg",
  },
];
...
export default function MoviesPage() {
  ...
  // Note: Do not mutate swipeTrigger on index changes.
  // This prevents the next card from accidentally consuming the previous trigger.
  ...
  return (
    <div className="mx-auto max-w-sm select-none">
      <div className="mb-4 flex items-center justify-between text-sm text-foreground/70">
        ...
        <span>{Math.max(MOVIES.length - (index + 1), 0)} left</span>
      </div>
...
function MovieSwipeCard({
  ...
  // Programmatic like/dislike from buttons (only act if trigger targets this card)
  ...
  useEffect(() => {
    ...
    // Don't clear timer in cleanup; Strict Mode would otherwise cancel it.
    return () => {};
  ...
  const onPointerUp = (e: React.PointerEvent) => {
    ...
    if (Math.abs(dx) > threshold) {
      ...
      const timer = setTimeout(() => onDecision(toLike), 280);
      // release capture and stop dragging
      try {
        (e.target as Element).releasePointerCapture?.(e.pointerId);
      } catch {}
      setDragging(false);
      // timer will be garbage collected after it fires; no cleanup needed here
    }
    // snap back
    setDx(0);
    setDragging(false);
    ...
    >
      <div
        className={[
          ...
          (isDragging && !leaving) || dx !== 0 ? "" : "",
          leaving ? "transition-transform duration-300 ease-out" : isDragging ? "" : "transition-transform duration-300 ease-out",
        ].join(" ")}
      ...
      >
        <div className="relative h-[65%] bg-black/5">
          ...
          <div className="absolute inset-0 pointer-events-none">
            <div
              className={`absolute top-3 left-3 rounded-md border px-2 py-1 text-xs font-semibold ${
                showLike ? "opacity-100" : "opacity-0"
              } transition-opacity bg-emerald-500/10 border-emerald-500/30 text-emerald-600`}
            >
              LIKE
            </div>
            <div
              className={`absolute top-3 right-3 rounded-md border px-2 py-1 text-xs font-semibold ${
                showNope ? "opacity-100" : "opacity-0"
              } transition-opacity bg-rose-500/10 border-rose-500/30 text-rose-600`}
            >
              NOPE
            </div>
          </div>
